...
| `Syllabus`      | `subject`, `topics` (`String[]`), `(examBoardId, subject)` unique                                                                                                                                                                                                                                                                                                                    | belongs to `ExamBoard` (required)                          |
...
regardless. `Syllabus(examBoardId, subject)` became unique the same way in `TAPS-2.15`, for the
same reason: its seed script needs to `upsert` idempotently by board+subject, and a board having
two rows for the same subject would itself be a data-integrity bug.
